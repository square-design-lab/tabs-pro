class sdlTabs {
    ...
    async function handleDOMReady() {
      ...
      try {
        if (typeof sdl$.initializeCodeBlocks === "function") {
          await sdl$.initializeCodeBlocks(this.el);
        }
        if (typeof sdl$.initializeEmbedBlocks === "function") {
          await sdl$.initializeEmbedBlocks(this.el);
        }
        if (typeof sdl$.initializeThirdPartyPlugins === "function") {
          await sdl$.initializeThirdPartyPlugins(this.el);
        }
        if (typeof sdl$.handleAddingMissingColorTheme === "function") {
          await sdl$.handleAddingMissingColorTheme();
        }
      } catch (error) {
  ...
  moveFromTargets() {
    ...
    this.initialInnerEl.querySelectorAll("button").forEach(btn => {
      ...
      if (!sdlTabs.originalPositions) {
        sdlTabs.originalPositions = new Map();
      }
  ...
  addTabNavigationScrollEvent() {
    ...
    const throttledHandleScroll = sdl$.throttle(handleScroll, 250);
  ...
  addTabsResizeEvent() {
    ...
    const throttledResize = sdl$.throttle(handleResize, 250);
  ...
  openTab(tabId) {
    ...
    sdl$?.emitEvent(`${sdlTabs.pluginTitle}:beforeOpenTab`, {
      tabId: tabId,
      instance: this,
    });
    ...
    sdl$?.emitEvent(`${sdlTabs.pluginTitle}:afterOpenTab`, {
      tabId: tabId,
      instance: this,
    });
